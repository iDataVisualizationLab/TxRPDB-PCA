...
const PMISMapModal: React.FC<MapModalProps> = ({ id, onClose, highway, county }) => {
  ...
  const handleMapLoaded = useCallback((map: any, view: MapView) => {
    ...
    if (view.popup) {
      ...
      view.popup.dockOptions = { position: "top-center", buttonEnabled: false, breakpoint: false }
    ...
    `,
      ...
      uniqueValueInfos: [
        ...
        },
        {
          value: "Very Poor",
          label: "Very Poor",
          symbol: { type: "simple-line", color: "rgba(239, 68, 68, 0.3)", width: 2 },
        },
        {
          value: "Poor",
          label: "Poor",
          symbol: { type: "simple-line", color: "rgba(249, 115, 22, 0.3)", width: 2 },
        },
        {
          value: "Fair",
          label: "Fair",
          symbol: { type: "simple-line", color: "rgba(234, 179, 8, 0.3)", width: 2 },
        },
        {
          value: "Good",
          label: "Good",
          symbol: { type: "simple-line", color: "rgba(34, 197, 94, 0.3)", width: 2 },
        },
        {
          value: "Very Good",
          label: "Very Good",
          symbol: { type: "simple-line", color: "rgba(21, 128, 61, 0.3)", width: 2 },
        },
      ],
    },
  })
  ...
  exactQuery.where = `(${whereExact}) AND COUNTY = '${county}'`;
  ...
  geoJSON_PMIS_LayerRef.current.queryExtent(exactQuery)
    .then(({ extent }) => {
      if (extent) {
        ...
        const fuzzyQuery = geoJSON_PMIS_LayerRef.current.createQuery();
        fuzzyQuery.where = `TX_SIGNED_HIGHWAY_RDBD_ID LIKE '%${baseNumber}%' AND COUNTY = '${county}'`;
...
function highlightFeatures(view: MapView, query: __esri.Query, logLabel: string) {
  ...
  layer.queryFeatures(query)
    .then(({ features }) => {
      ...
      features.forEach(feature => {
        // Add thick black outline underneath everything - INCREASED from 10 to 12
        hl.add(new Graphic({
          geometry: feature.geometry,
          symbol: {
            type: "simple-line",
            color: [0, 0, 0,],  // Black outline
            width: 10,              // Increased from 10 to 12
            cap: "round",
            join: "round"
          }
        }));
      ...
      features.forEach(feature => {
        ...
        // main line - INCREASED from 4 to 7
        hl.add(new Graphic({
          geometry: feature.geometry,
          symbol: {
            type: "simple-line",
            color,
            width: 7,           // Increased from 4 to 7
            cap: "round",
          },
  ...
  // Make sure currentHighwaySegments is updated properly
  const calculateHistogram = useCallback((features: __esri.Graphic[]) => {
    ...
    // Store the current highway segments for legend interactions
  ...
  // Replace your handleLegendInteraction function with this corrected version
  const handleLegendInteraction = useCallback(
    (category: string | null, action: "hover" | "click") => {
      if (!viewRef.current) return;

      // Get the highlight layer that contains our specific highway segments
      const highlightLayer = viewRef.current.map.findLayerById("highlight-layer") as GraphicsLayer;
      
      if (!highlightLayer || !highlightLayer.graphics.length) {
        console.log("No highlighted segments found for this highway");
        return;
      }

      // Get only the main line graphics (those with attributes - the colored segments)
      const mainLineGraphics = highlightLayer.graphics.filter(graphic => 
        graphic.attributes && graphic.attributes.TX_CONDITION_SCORE !== undefined
      );

      if (mainLineGraphics.length === 0) {
        console.log("No main line graphics found with attributes");
        return;
      }

      // For hover actions - ONLY if no category is currently selected (clicked)
      if (action === "hover" && !selectedCategory) {
        if (!category) {
          // Reset hover - show all segments normally
          if (currentHighwaySegments.length > 0) {
            rebuildHighlightLayer(currentHighwaySegments, null);
          }
          return;
        }

        // Apply hover filter temporarily
        rebuildHighlightLayer(currentHighwaySegments, category, true);

        // After 5 seconds, reset ONLY if still no selection (no clicked category)
        setTimeout(() => {
          if (viewRef.current && !selectedCategory && currentHighwaySegments.length > 0) {
            rebuildHighlightLayer(currentHighwaySegments, null);
          }
        }, 5000);
      }
      // For click actions - these should be PERMANENT until another click
      else if (action === "click") {
        // Toggle selection if clicking the same category
        if (category === selectedCategory) {
          setSelectedCategory(null);
          // Reset all segments to normal view
          rebuildHighlightLayer(currentHighwaySegments, null);
        } else {
          setSelectedCategory(category);
          if (category) {
            // Filter segments based on selected category - NO TIMEOUT
            rebuildHighlightLayer(currentHighwaySegments, category, false);
          }
        }
      }
      // If there's already a selected category, ignore hover actions
      // This prevents hover from interfering with clicked selections
    },
    [selectedCategory, currentHighwaySegments]
  );

  // Add this new helper function to rebuild the highlight layer
  const rebuildHighlightLayer = useCallback((features: __esri.Graphic[], filterCategory: string | null, isHover = false) => {
    ...
    // Filter features first if there's a category selected
    ...
    // Only draw the features that should be visible
    featuresToShow.forEach(feature => {
      ...
      // Black outline - INCREASED from 6 to 12
      highlightLayer.add(new Graphic({
        geometry: feature.geometry,
        symbol: {
          type: "simple-line",
          color: [0, 0, 0, 1],
          width: 10,              // Increased from 6 to 12
          cap: "round",
          join: "round"
        }
      }));

    // White glow - INCREASED from 5.5 to 9
    highlightLayer.add(new Graphic({
      geometry: feature.geometry,
      symbol: {
        type: "simple-line",
        color: [255, 255, 255, 0.5],
        width: 5.5,               // Increased from 5.5 to 9
      }
    }));

    // Colored line - INCREASED from 4 to 7
    highlightLayer.add(new Graphic({
      geometry: feature.geometry,
      symbol: {
        type: "simple-line",
        ...
        >
          ...
          <div className="h-[calc(100%-48px)] w-full overflow-hidden">
            <DynamicMapComponent
              ...
              onMapLoaded={(map, view) => {
                handleMapLoaded(map, view);

                const pmisLayer = view.map.allLayers.find((layer) => layer.title === "PMIS Data")
                if (pmisLayer) {
          ...
          <div className="absolute bottom-4 right-4 bg-white/95 backdrop-blur-sm rounded-xl p-4 shadow-xl border border-gray-200/50 text-xs transition-all duration-300 hover:shadow-2xl" style={{ minWidth: "220px" }}>
            <div className="flex items-center justify-between mb-3 pb-2 border-b border-gray-200">
              ...
              <div className="group relative ml-auto">
                ...
                <div className="absolute bottom-full right-0 mb-2 w-56 transform scale-0 group-hover:scale-100 transition-transform origin-bottom z-50">
                  <div className="bg-gray-900 text-white text-sm p-3 rounded shadow-lg">
                    <p className="mb-2 font-medium text-base">Highway-Specific Legend:</p>
            ...
            <div className="space-y-2 legend-container">
              {[
                { id: "very-good", color: "rgb(21,128,61)", label: "Very Good" },
                { id: "good", color: "rgb(34,197,94)", label: "Good" },
                { id: "fair", color: "rgb(234,179,8)", label: "Fair" },
                { id: "poor", color: "rgb(249,115,22)", label: "Poor" },
                { id: "very-poor", color: "rgb(239,68,68)", label: "Very Poor" },
                { id: "invalid", color: "rgb(200,200,200)", label: "Invalid" }
              ].map((item) => {
                ...
                <span className="font-bold text-gray-800">
                  {selectedCategory 
                    ? (() => {
                        ...
                        const categoryData = histogramData.find(item => item.category === selectedCategory);
                        if (!categoryData || categoryData.count === 0) return "N/A";
                        ...
                        return weights[selectedCategory as keyof typeof weights]?.toString() || "N/A";
                      })()
